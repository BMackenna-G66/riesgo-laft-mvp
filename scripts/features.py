...
def _fill_nulls(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["compliance_status", "onboarding_status", "country_code"]:
        if col in df.columns:
...
def _derived_variables(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "customer_created_at" in df.columns:
        ...
        df["customer_age_days"] = (pd.Timestamp(today) - df["customer_created_at"]).dt.days.fillna(0).clip(lower=0)
    else:
    ...
    if "last_operation_date" in df.columns:
        ...
        df["days_since_last_op"] = (pd.Timestamp(today) - df["last_operation_date"]).dt.days.fillna(999)
    else:
    ...
    df["beneficiary_per_trx"] = np.where(
        df["trx_180d"] > 0,
        df.get("unique_beneficiaries_180d", 0) / df["trx_180d"],
        0.0,
    )

    # Indicador cliente empresa (si tiene company_id, es B2B)
    if "company_id" in df.columns:
        df["es_empresa"] = df["company_id"].notna().astype(int)
    else:
        df["es_empresa"] = 0

    # Compliance flag: clientes en revisión = mayor riesgo potencial
    if "compliance_status" in df.columns:
        df["compliance_risk_flag"] = df["compliance_status"].isin(
            ["UNDER_COMPLIANCE_REVIEW", "BLOCKED", "SUSPENDED"]
        ).astype(int)
    else:
...
def build_features(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    # Normalizar variables derivadas también
